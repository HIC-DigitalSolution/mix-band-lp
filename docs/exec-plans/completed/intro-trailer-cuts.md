...
**2026-09-09 追記: 1カット目を差し替えた。**依頼者から「変わってないね」と指摘された。
原因は1カット目が引きの店内で、**FVも引きの集合写真なので落差が無かった**こと。
節に入っても絵が変わったと分からない。**球が空中にあるビアポン**を1カット目にして、
最初から動きのある絵で始めるようにした。順番は 01 ビアポン → 02 店内の引き・乾杯 →
03 カウンターの2人 → 04 寄り＋題字。`intro-is-a-trailer-of-cuts` として不変条件にした。

**まだ残る弱さ** —— 4カット目（題字＋本文）は以前の静止パネルと似た絵になる。
そこで止まって見ている人には変化が伝わらない。締め方には手を入れる余地がある。
